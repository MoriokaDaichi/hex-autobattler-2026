...
- **ユーザー不在時(就寝中等)の自動進行**: 「寝るので夜間は自動でタスクを進めて」のような指示を受けた場合、Notionタスクキューを締切順に、上記の役割分担・レビューチェーンに沿って自動で進めて構わない。判断が必要な細かい点（実装方針の些細な選択等）は都度確認を待たず、自分で妥当なデフォルトを選んで進めてよい。ただし**pushの都度確認ルールはこの「自動で進めて」という指示によって免除されない**（一般的な自動化の許可を、個別具体のpush承認の代わりに解釈してはならない）。コミットまでは進めてよいが、pushはユーザーが戻って直接確認するまで保留し、ユーザー復帰時にまとめて何が完了・コミット済みでpush待ちかを報告する。ただし、そのタスク自体が既存コードに無い新しいゲームメカニクスの設計を要する（=実装細部の裁量を超える）場合は、夜間でも独断で仕様を作り切らず、そこで手を止めてユーザーの起床を待つ。
- **要件→設計→実装→レビューのフェーズ運用**: 個々のタスクは「要件定義→設計→実装→レビュー」の4フェーズで進める。フェーズ間の引き継ぎは全てMarkdown文書で行い、**次のフェーズの担当がその文書だけを読めば(それ以前のチャット履歴を遡らずに)着手できる**よう自己完結させる。
  - 保存場所: `docs/tasks/<タスク名の英語kebab-case>/`配下(例: `docs/tasks/item-acquisition-equip/`)。
  - 要件定義フェーズの成果物: `intent.md`(背景・現状・目的・要求仕様・受け入れ条件)。
  - 設計フェーズの成果物: `plan.md`(具体的な技術設計: 追加/変更するファイル、データ構造、UI配置、既知の制約への対応方針、未解決の懸念点があればそれも)。
  - 実装フェーズは`intent.md`/`plan.md`を読んだ上で着手し、コミットメッセージ自体が実装内容の記録を兼ねる。
  - レビューフェーズ(デバッグ確認セッションでの検証等)では、`plan.md`の内容と実際の実装が一致しているかも確認観点に含める。
