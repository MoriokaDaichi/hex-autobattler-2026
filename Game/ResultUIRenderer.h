...
/// 2. color.wによるアルファブレンドが機能しない(常に不透明)。点滅表現はアルファフェードではなく
///    一定間隔で描画自体をON/OFFする方式で行う(TitleUIRendererのPRESS [A] TO STARTと同じ)。
/// </summary>
...
public:
	...
	void DrawGameOver(RenderContext& rc, int reachedRound, int totalRounds, float deltaTime);
