...
namespace
{
	...
	// MeasureString相当が無いため、1文字あたりの見た目幅を概算する(ResultUIRendererのCenteredStartX
	// で校正した「スケール1.0基準で概ね23px(半角換算)」の値を流用)。全角文字は半角の2倍幅とみなす。
	const float kApproxCharWidthAtScale1 = 23.0f;
	...
	float EstimateWidth(const std::wstring& text, float scale)
	{
		...
		return count * kApproxCharWidthAtScale1 * scale;
	}
}
